...
class SpotifyRequest(APIRequest):
    ...
    def search_artists(self, query, n=10):
        ...
        sorted_artists = [{"name": x["name"], "uri": x["uri"]} for x in sorted_artist_dicts]
    ...
    def search_tracks(self, query, n=10):
        ...
        sorted_tracks = [
            {"name": x["name"], "artist": x["artists"][0]["name"], "uri": x["uri"]} for x in sorted_track_dicts
        ]
...
# figure out how to add username to config and then figure out scoping and then write methods for all use cases.
#  god damn fuck this package.
