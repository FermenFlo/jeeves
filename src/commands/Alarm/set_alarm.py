...
class SetAlarm(Command):
    ...
    def run(self, jeeves):
        ...
        sorted_datetimes = sorted(entities['datetime'], key = lambda x: -x['confidence']) if "datetime" in entities else []
        sorted_durations = sorted(entities['duration'], key = lambda x: -x['confidence']) if "durations" in entities else []
